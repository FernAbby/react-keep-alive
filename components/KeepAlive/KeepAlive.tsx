...
type IKeeperElement = HTMLDivElement | null

const prevRefs: {
  [key: string]: IKeeperElement
} = {}

const KeepAlive: FC<PropsWithChildren<IKeepAliveProps>> = (props) => {
  const keepAliveRef = useRef<IKeeperElement>(null)
  ...
  const activated = useMemoizedFn(async () => {
    ...
    if (pageContent && keepAliveRef.current) {
  ...
  // 缓存状态: 页面重新缓存入Keeper内
  const deactivated = useMemoizedFn((_cacheKey: string, keepContainerElement: IKeeperElement) => {
    // 页面重新缓存入Keeper内
